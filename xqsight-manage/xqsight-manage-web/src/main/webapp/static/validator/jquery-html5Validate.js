			...
			isAllpass : function(elements, options) {
				...
				elements.each(function() {
					...
					if (allpass == false || el.disabled || type == 'submit' || type == 'reset' || type == 'file' || type == 'image')
						return;
					// 需要验证的有
					// input文本框, type, required, pattern, max,
					// min以及自定义个数限制data-min, data-max
					// radio, checkbox
					// select
					// textarea
					// 先从特殊的下手，如单复选框
					if (type == "radio" && isRequired) {
						...
						var eleRadios = el.name ? $("input[type='radio'][name='" + el.name + "']") : $(el), radiopass = false;
	...
	$.fn.extend({
		...
		testRemind : function(content, options) {
			...
			this.remind.css({
				...
				// this.remind.outerWidth()) * 0.5 +
				// /*因为三角位置造成的偏移*/ this.remind.outerWidth() *
				// (50 - parseInt(align)) / 100
				// 等于下面这个：
