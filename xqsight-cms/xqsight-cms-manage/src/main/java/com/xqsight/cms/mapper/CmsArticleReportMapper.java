...
public interface CmsArticleReportMapper {
	...
	@Insert(" insert into cms_article_report(assocication_id,report_type,report_content,deal_status,active,create_time,create_user_id,remark)values(#{associcationId,jdbcType=NUMERIC},#{reportType,jdbcType=NUMERIC},#{reportContent,jdbcType=VARCHAR},#{dealStatus,jdbcType=NUMERIC},#{active,jdbcType=NUMERIC},#{createTime,jdbcType=TIMESTAMP},#{createUserId,jdbcType=VARCHAR},#{remark,jdbcType=VARCHAR})")
	void saveCmsArticleReport(CmsArticleReport cmsArticleReport);
	
	@Update(" update cms_article_report set deal_status=0,update_time=#{updateTime,jdbcType=TIMESTAMP},update_user_id=#{updateUserId,jdbcType=VARCHAR},remark=#{remark,jdbcType=VARCHAR} where report_id=#{reportId,jdbcType=NUMERIC}")
	...
	@Select(" select report_id,report_content,assocication_id,report_type,deal_status,active,create_time,create_user_id,update_time,update_user_id,remark from cms_article_report where report_id=#{reportId,jdbcType=NUMERIC}")
