...
public interface CmsArticleReportMapper {
	...
	@Select(" SELECT REPORT_ID,ASSOCICATION_ID,REPORT_TYPE,DEAL_STATUS,ACTIVE,CREATE_TIME,CREATE_OPR_ID,UPDATE_TIME,UPD_OPR_ID,REMARK FROM CMS_ARTICLE_REPORT WHERE REPORT_ID=#{reportId,jdbcType=NUMERIC}")
	CmsArticleReport queryCmsArticleReportById(@Param("reportId") Long reportId);

	@Select(" SELECT REPORT_ID,ASSOCICATION_ID,REPORT_TYPE,DEAL_STATUS,ACTIVE,CREATE_TIME,CREATE_OPR_ID,UPDATE_TIME,UPD_OPR_ID,REMARK FROM CMS_ARTICLE_REPORT ORDER BY CREATE_TIME DESC")
