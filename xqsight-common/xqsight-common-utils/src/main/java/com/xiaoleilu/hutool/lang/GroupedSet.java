...
public class GroupedSet extends HashMap<String, LinkedHashSet<String>>{
	...
	private Logger logger = LogManager.getLogger(GroupedSet.class);
	...
	public boolean init(URL groupedSetUrl, String charset) {
		...
		try {
			...
			logger.warn("User custom charset [{}] parse error, use default charset: [{}]", charset, Setting.DEFAULT_CHARSET);
	...
	synchronized public boolean load(URL groupedSetUrl) {
		...
		try {
			...
			logger.error("Load GroupSet error!", e);
			return false;
		} finally {
