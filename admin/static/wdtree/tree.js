    ...
    $.fn.treeview = function (settings) {
        var dfop =
            {
                 ...
                 */
                cbiconpath: "/portal/static/wdtree/images/icons/",
        ...
        function buildtree(data, ht) {
            ...
            if (data && data.length > 0) {
                ...
                asnyloadc(null, false, function (retData) {
                    if(retData.status == -1) return false;
                    data = retData.data;
                    if (data && data.length > 0) {
                        treenodes = data;
                        dfop.data = data;
        ...
        function buildnode(nd, ht, deep, path, isend) {
            //add xqsight tree style support
            if(nd.children != undefined && nd.children.length > 0){
                nd.hasChildren = true;
                nd.isexpand = true;
                nd.ChildNodes = nd.children;
            }
                ...
                if ($(et).hasClass("bbit-tree-elbow-plus") || $(et).hasClass("bbit-tree-elbow-end-plus")) {
                    ...
                    if (ul.hasClass("bbit-tree-node-ct")) {
                        ...
                        if (item.complete) {
                            ...
                            asnyloadc(item, true, function (retData) {
                                if(retData.status == -1) return false;
                                data = retData.data;
                                item.complete = true;
