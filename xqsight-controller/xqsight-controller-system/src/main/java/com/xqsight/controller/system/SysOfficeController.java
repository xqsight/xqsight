...
public class SysOfficeController {
    ...
    public Object save(SysOffice sysOffice) {
        if(StringUtils.equals(sysOffice.getParentId(),"0")){
    ...
    public Object queryTree(String officeType) {
        ...
        if (StringUtils.isNotBlank(officeType)) {
            propertyFilters = PropertyFilterBuilder.create().matchTye(MatchType.EQ)
                    .propertyType(PropertyType.I).add("office_type", officeType).end();
        }

        List<SysOffice> sysOffices = sysOfficeService.search(propertyFilters);
        sysOffices = new TreeSupport<SysOffice>().generateTree(sysOffices);
        return MessageSupport.successDataMsg(sysOffices, "查询成功");
    }
